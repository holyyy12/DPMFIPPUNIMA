const PLACEHOLDERS = [
  'YOUR_NEW_PROJECT_REF',
  'YOUR_NEW_PROJECT_ANON_KEY',
  'SET_IN_SECRET_MANAGER_ONLY',
];
...
export function validateEnvironment(env) {
  ...
  const publicKeys = Object.keys(env).filter((key) =>
    key.startsWith('NEXT_PUBLIC_'),
  );

  if (
    publicKeys.some((key) =>
      /SERVICE_ROLE|SECRET|PEPPER|ENCRYPTION_KEY|CRON/i.test(key),
    )
  )
    errors.push('Secret-like key exposed through NEXT_PUBLIC_*');
  if (appEnv !== 'local') {
    for (const key of [
    ...
    if (actual && expected && actual !== expected)
      errors.push(
        'Supabase project ref does not match EXPECTED_SUPABASE_PROJECT_REF',
      );
  }
  return {
    ok: errors.length === 0,
    errors,
    appEnv,
    actualProjectRef: actual,
    expectedProjectRef: expected ?? null,
  };
}
