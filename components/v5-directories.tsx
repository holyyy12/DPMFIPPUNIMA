...
type ContentMetadata = {
  ...
  topic?: string;
  period?: string;
  program?: { unit?: string; media?: string; image?: string };
};
...
function programView(item: PublicContent) {
  ...
  return {
    ...item,
    copy: item.summary,
    unit: detail.unit ?? item.unit_name ?? item.organization_name ?? 'DPM FIPP',
    media: detail.media ?? 'photo',
    image: publicAssetUrl(item),
    progress: item.progress_percent ?? 0,
    success: item.success_percent ?? 0,
    updateNote: item.public_note || 'Belum ada pembaruan publik.',
  };
}
function Hero({
  eyebrow,
...
export function ProgramsPage() {
  ...
  const programs = data.contents
    .filter((item) => item.content_type === 'program')
    .map(programView);
  return (
    <PublicFrame>
      <Hero eyebrow="PROGRAM KERJA" title="Pantau Kinerja DPM FIPP." copy="" />
      <section className="v5-shell v5-program-grid">
        {programs.map((program) => (
          <article key={program.id}>
            ...
            >
              <span>
                {program.media === 'video' ? <Play /> : <Images />}
                {program.media}
              </span>
            </div>
            <div>
              <small>{program.unit}</small>
              <h2>{program.title}</h2>
              <p>{program.copy}</p>
              <label>
                Progress program <b>{program.progress}%</b>
                <i>
                  <em style={{ width: `${program.progress}%` }} />
                </i>
              </label>
              <label>
                Indikator keberhasilan <b>{program.success}%</b>
                <i>
                  <em style={{ width: `${program.success}%` }} />
                </i>
              </label>
        ...
        {error && (
          <p className="v5-filter-empty">Data program belum dapat dimuat.</p>
        )}
      </section>
    </PublicFrame>
  );
...
export function ProgramDetailPage({ slug }: { slug: string }) {
  ...
  if (loading)
    return (
      <PublicFrame>
        <section className="v5-shell v5-program-detail">
          <p>Memuat program…</p>
        </section>
      </PublicFrame>
    );
  if (!program)
    return (
      <PublicFrame>
        <section className="v5-shell v5-program-detail">
          <Link href="/program">
            <ArrowLeft /> Semua Program
          </Link>
          <h1>Program tidak ditemukan</h1>
          <p>Program ini belum diterbitkan melalui Portal Admin.</p>
        </section>
      </PublicFrame>
    );
  return (
    <PublicFrame>
      <section className="v5-shell v5-program-detail">
        ...
        <div
          className="v5-program-detail-hero"
          style={{
            backgroundImage: `linear-gradient(90deg,#043d30dd,#043d3040),url(${program.image})`,
          }}
        >
          <span>
            {program.media === 'video' ? <Play /> : <Images />}
        ...
        <div className="v5-program-detail-grid">
          <article>
            <h2>Publikasi Program</h2>
            <p>{program.updateNote}</p>
            <ContentGallery items={contentMedia(program)} />
          </article>
          <aside>
            <h2>Progres</h2>
            ...
            <p>
              <CheckCircle2 /> Indikator keberhasilan: <b>{program.success}%</b>
            </p>
            <p>
              <CheckCircle2 /> Pembaruan terakhir:{' '}
              {formatPublicDate(
                program.progress_updated_at ?? program.updated_at,
              )}
            </p>
          </aside>
        </div>
...
export function SightPage() {
  ...
  const studies = data.contents.filter(
    (item) => item.content_type === 'd-sight',
  );
  ...
  return (
    <PublicFrame>
      <Hero
        ...
        copy="D-SIGHT membantu mahasiswa memahami isu melalui kajian, hasil survei, dan berita yang relevan."
      />
      ...
      <section className="v5-shell v5-domain-section" id="kajian">
        ...
        <div className="v5-study-cards">
          {studies.map((item) => (
            <article key={item.id}>
              <span>{categoryOf(item, 'Kajian')}</span>
              <h3>{item.title}</h3>
              <p>Diterbitkan {formatPublicDate(item.published_at)}</p>
        ...
        {!loading && !studies.length && (
          <p className="v5-filter-empty">
            Belum ada kajian yang dipublikasikan.
          </p>
        )}
      </section>
      <section className="v5-shell v5-domain-section" id="survei">
        <header>
          ...
          <div>
            ...
            <p>
              Pilih isu dan lihat jumlah respons tanpa data pribadi responden.
            </p>
          </div>
        </header>
        <PublicSurveys />
        ...
        <div className="v5-news-grid">
          {news.map((item) => (
            <Link href={`/berita/${item.slug}`} key={item.id}>
              ...
              <span>
                <b>{item.title}</b>
                <small>{formatPublicDate(item.published_at)}</small>
              </span>
            </Link>
          ))}
        </div>
...
export function TracePage() {
  ...
  const categories = [
    ...new Set(items.map((item) => categoryOf(item, 'Publikasi'))),
  ].sort();
  const periods = [
    ...new Set(
      items
        .map((item) => metadata(item).period ?? data.period.name ?? '')
        .filter(Boolean),
    ),
  ].sort();
  const filtered = useMemo(
    () =>
      items.filter((item) => {
        ...
        return (
          (category === 'Semua' ||
            categoryOf(item, 'Publikasi') === category) &&
          (period === 'Semua' || itemPeriod === period) &&
          `${item.title} ${item.summary}`
            .toLowerCase()
            .includes(query.toLowerCase())
        );
      }),
    [items, query, category, period, data.period.name],
  );
  return (
    <PublicFrame>
      <Hero
          ...
          {filtered.map((item) => {
            ...
            return (
              <article key={item.id}>
                ...
                <div>
                  <small>{categoryOf(item, 'Publikasi')}</small>
                  <h2>{item.title}</h2>
                  <p>
    ...
    () =>
      items.filter((item) => {
        ...
        return (
          ...
          (period === 'Semua' || itemPeriod === period) &&
          `${owner} ${item.title} ${item.summary}`
            .toLowerCase()
            .includes(query.toLowerCase())
        );
      }),
    [items, query, organization, period, data.period.name],
  );
  return (
    <PublicFrame>
      <Hero
    ...
    <PublicFrame>
      ...
      <section className="v5-shell v5-ormawa-grid">
        {data.organizations.map((item) => {
          const programs = data.contents.filter(
            (content) =>
              content.content_type === 'program' &&
              content.organization_id === item.id,
          );
          return (
            <article key={item.id}>
              <span>
        ...
        {!loading && !data.organizations.length && (
          <p className="v5-filter-empty">
            Belum ada ORMAWA aktif pada database.
          </p>
        )}
      </section>
    </PublicFrame>
...
export function OrmawaProfile({ slug }: { slug: string }) {
  ...
  if (!item)
    return (
      <PublicFrame>
        <section className="v5-shell">
          <h1>ORMAWA tidak ditemukan</h1>
          <p>Halaman organisasi ini belum diterbitkan melalui Portal Admin.</p>
          <Link href="/ormawa">Kembali ke daftar ORMAWA</Link>
        </section>
      </PublicFrame>
    );
  const programs = data.contents
    ...
    .map(programView);
  const gallery = data.contents.filter(
    (content) =>
      content.organization_id === item.id && content.featured_object_path,
  );
  return (
    <PublicFrame>
