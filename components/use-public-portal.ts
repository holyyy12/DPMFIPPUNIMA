...
export function usePublicPortal() {
  ...
  const reload = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await fetch('/api/public/portal', { cache: 'no-store' });
      const payload = (await response.json()) as {
        ok: boolean;
        data?: PublicPortalSnapshot;
        message?: string;
      };
      if (!response.ok || !payload.ok || !payload.data)
        throw new Error(payload.message ?? 'Data publik tidak tersedia.');
