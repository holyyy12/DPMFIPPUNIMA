...
export function AdminShell({children}:{children:React.ReactNode}){
 ...
 return <main className={`v4-admin ${open?'menu-open':''}`}><button className="v4-admin-backdrop" onClick={()=>setOpen(false)} aria-label="Tutup menu"/>
  <aside><header><Link href="/"><img src="/dpm-crest.png" alt="Lambang DPM FIPP UNIMA"/><span><b>DPM FIPP UNIMA</b><small>ADMIN</small></span></Link><button onClick={()=>setOpen(false)}><X/></button></header>
   <div className="v4-admin-profile"><span>{displayName.split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase()}</span><p><b>{displayName}</b><small>{role}</small><em>● Online</em></p></div>
   <nav>{items.map(([href,label,Icon])=>{const active=pathname===href||pathname.startsWith(href+'/');return <Link key={href} href={href} className={active?'active':''} onClick={()=>setOpen(false)}><Icon/>{label}{label==='Notifikasi'&&unread>0&&<i>{unread}</i>}</Link>})}</nav>
  ...
  <section><header className="v4-admin-top"><div><button onClick={()=>setOpen(true)}><Menu/></button><Link href="/admin/dashboard">Beranda</Link><span>›</span><p>{names[pathname]??'Portal Admin'}</p></div><div><span><ShieldCheck/> {displayName}</span><button onClick={()=>location.assign('/admin/notifications')} aria-label="Buka notifikasi"><Bell/>{unread>0&&<i>{unread}</i>}</button></div></header>{children}<footer className="v4-admin-foot">© 2026 DPM FIPP UNIMA. Semua hak dilindungi.<span>Versi 1.0.0</span></footer></section>
 </main>
}
