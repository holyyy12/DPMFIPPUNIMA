...
export function ModerationWorkspace() {
  ...
  const [message, setMessage] = useState('Memuat antrean aman…');
  async function load() {
    ...
    setMessage(result.ok ? '' : (result.message ?? 'Antrean belum tersedia.'));
  }
  useEffect(() => {
    void load();
  }, []);
  async function decide(
    ...
    const detail = window.prompt('Catatan keputusan (opsional)', '') ?? '';
    ...
    setMessage(
      result.ok
        ? 'Keputusan tersimpan dan dicatat pada audit log.'
        : (result.message ?? 'Keputusan gagal.'),
    );
    ...
    <div className="admin-content admin-module-content">
      ...
      <section className="admin-module-metrics">
        <article>
          ...
          <b>Perlu keputusan</b>
          <small>Pending atau dilaporkan</small>
        </article>
        <article>
          <strong>
            {items.reduce((sum, item) => sum + Number(item.report_count), 0)}
          </strong>
          <b>Laporan terbuka</b>
      ...
      <section className="admin-panel admin-workspace">
        <header>
          <div>
            ...
            <h2>Tinjau berdasarkan konteks</h2>
          </div>
        </header>
        <div className="moderation-list">
          {items.length === 0 && !message ? (
            <div className="comment-empty">
              Tidak ada komentar yang menunggu keputusan.
            </div>
          ) : (
            items.map((item) => (
              <article key={item.comment_id}>
                <header>
                  ...
                  {Number(item.report_count) > 0 && (
                    <span>
                      <Flag /> {item.report_count} laporan
                    </span>
                  )}
                </header>
                <p>{item.body}</p>
                {item.report_categories.length > 0 && (
                  <small>Kategori: {item.report_categories.join(', ')}</small>
                )}
                <footer>
                  <button onClick={() => decide(item, 'published', 'approved')}>
                    <CheckCircle2 /> Publikasikan
                  </button>
                  <button onClick={() => decide(item, 'hidden', 'other')}>
                    <EyeOff /> Sembunyikan
                  </button>
                  <button
                    className="danger"
                    onClick={() => decide(item, 'rejected', 'other')}
                  >
                    Tolak
                  </button>
                </footer>
