...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!pepper || pepper.includes('SET_IN_'))
    ...
    >(
      'track_ddas',
      { p_ticket: input.ticket, p_secret_hash: secretHash },
      { noStore: true },
    );
    if (!rows.length)
      return Response.json(
        {
          ...
          code: 'NOT_VERIFIED',
          message:
            'Kode tidak dapat diverifikasi. Periksa kembali nomor tiket dan kode pelacakan.',
          requestId,
        },
        { status: 404, headers },
      );
    ...
    return Response.json(
      {
        ok: false,
        code: unavailable ? 'SERVICE_UNAVAILABLE' : 'NOT_VERIFIED',
        message: unavailable
          ? 'Pelacakan belum diaktifkan pada backend greenfield baru.'
          : 'Kode tidak dapat diverifikasi. Periksa kembali nomor tiket dan kode pelacakan.',
        requestId,
      },
