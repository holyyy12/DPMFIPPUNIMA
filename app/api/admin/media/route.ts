...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || file.size < 1 || file.size > MAX_BYTES)
      return Response.json(
        { ok: false, message: 'File wajib diisi dan maksimal 20 MB.' },
        { status: 400 },
      );
    const bucket =
      form.get('bucket') === 'private-media' ? 'private-media' : 'public-media';
      ...
      return Response.json(
        {
          ok: false,
          message:
            'Upload ditolak. Pastikan akun memiliki izin media dan file sesuai batas.',
        },
    ...
    return Response.json(
      { ok: false, message: 'File gagal diunggah dan dicatat.' },
      { status: 400 },
    );
  }
}
