...
export async function GET() {
  try {
    ...
    return Response.json(
      {
        ok: false,
        message: 'Data Portal Admin tidak dapat dimuat dari Supabase.',
      },
      { status: 503 },
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === 'user.invite_request') {
      ...
      if (!parsed.success)
        return Response.json(
          {
            ok: false,
            message: 'Nama, email, role, dan unit harus diisi dengan benar.',
          },
          { status: 400 },
        );
    ...
    if (input.action === 'unit.save') {
      ...
      if (!data.ok) {
        const messages: Record<string, string> = {
          ...
          NO_ACTIVE_PERIOD:
            'Belum ada periode aktif. Aktifkan periode kepengurusan sebelum menambah unit.',
          DUPLICATE_UNIT:
            'Kode atau nama unit sudah digunakan pada periode aktif.',
        };
        return Response.json(
          {
            ok: false,
            message:
              messages[data.error ?? ''] ?? 'Unit tidak dapat ditambahkan.',
          },
          { status: 400 },
        );
      }
    ...
    const data = await supabaseRpc<{ ok: boolean; id?: string }>(
      extended ? 'admin_portal_extended_action' : 'admin_portal_action',
      {
    ...
    return Response.json(
      { ok: false, message: 'Aksi gagal. Periksa izin dan data yang diisi.' },
      { status: 400 },
    );
  }
}
