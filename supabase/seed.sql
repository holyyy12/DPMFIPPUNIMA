...
('secretary','Secretary','Secretariat, review, approval, and publication operations',true),
('organization_unit','Organization Unit','Dynamic unit-scoped role; concrete units are stored in dpm_units',true),
...
from public.roles r join public.permissions p on p.key in ('content.read.unit','content.update.unit','media.create.all','approval.submit.unit','approval.read.unit')
where r.key='organization_unit'
on conflict(role_id,permission_id) do update set effect=excluded.effect;

insert into public.role_permissions(role_id,permission_id,effect)
select r.id,p.id,'allow'::public.permission_effect
from public.roles r join public.permissions p on p.key in ('content.read.all','content.update.all','content.publish.all','media.create.all','media.update.all','approval.read.all','approval.decide.secretary')
where r.key='secretary'
on conflict(role_id,permission_id) do update set effect=excluded.effect;

insert into public.role_permissions(role_id,permission_id,effect)
select r.id,p.id,'allow'::public.permission_effect
from public.roles r join public.permissions p on p.key='approval.read.all'
...
('iam','organization_unit_approval_sla','{"secretaryHours":36,"superAdminHours":12,"fallbackApproval":true,"totalHours":48}',1,false)
